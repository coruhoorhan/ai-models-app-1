...
export function TopNav() {
  return (
    <nav className="w-full h-[64px] bg-canvas border-b border-hairline flex justify-center sticky top-0 z-index-sticky">
        ...
        <div className="hidden md:flex items-center gap-lg">
          {['DASHBOARD', 'MODELS', 'RANKINGS', 'PRICING', 'CHAT', 'DOCS'].map((link) => (
            <Link
              ...
              className="text-body-sm text-subtle hover:text-ink tracking-[0.02em] font-medium transition-fast uppercase focus-ring rounded-sm px-2 py-1"
            >
              {link}
            </Link>
          ))}
