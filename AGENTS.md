...
- [ ] Kodlanan piksel/genişlik/renk değerleri PAGES.md ve DESIGN.md'deki sayılarla **birebir** eşleşiyor mu — görsel olarak "yakın" değil, sayısal olarak aynı. (Örn. blueprint "480px" diyorsa kodda `max-w-[420px]` gibi yaklaşık bir değer kabul edilmez.)
- [ ] PAGES.md'de tarif edilen her öğe (dekoratif elemanlar dahil — ikon rozetleri, network grafiği, logo şeridi gibi "küçük" görünen detaylar) gerçekten kodlanmış mı, yoksa görsel olarak "yeterince benzer" diye atlanmış mı?
...
- **Tam Genişlik (100% Width) Zorunluluğu (ÇOK KRİTİK):** Sayfalarda `max-w-7xl`, `max-w-6xl` veya `mx-auto` gibi ekranı ortalayan veya genişliği sınırlayan (kompakt) Tailwind sınıflarını KESİNLİKLE KULLANMA. Ekranlar her zaman %100 tam genişlikte (full-width) olmalıdır. Tüm sayfalar standart olarak `<div className="w-full p-4 2xl:p-6 flex flex-col gap-6">` wrapper'ı ile başlamalıdır (DESIGN.md `{spacing.md}`/`{spacing.lg}` token'larına karşılık gelen değerler). Ana kaydırma (scroll) işlemi her zaman `Shell` veya ana layout component'inin sorumluluğundadır.
